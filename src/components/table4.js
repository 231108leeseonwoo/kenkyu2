...
const convertTimestampToDate = (timestamp) => {
  ...
  const handleBetResult = async () => {
    ...
    for (const eventId in selectedOdds) {
      ...
      // 試合の詳細情報を格納
      let result = "開始前"; // 初期値として"開始前"を設定
  
      // oddsData.choices[selectedChoiceIndex].winning が存在するかチェック
      if (oddsData?.choices?.[selectedChoiceIndex]?.winning !== undefined) {
        // 試合が開始しており、結果がわかる場合
        result = oddsData.choices[selectedChoiceIndex].winning ? '的中' : '外れ';
      }
    ...
    // }
    predictedAmount = betAmount * totalOdds;
