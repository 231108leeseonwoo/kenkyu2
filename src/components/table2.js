...
export default function Table2({ data }) {
  ...
  // oddsはオブジェクトなので、キーを使ってアクセス
  const events = Array.isArray(data.odds) ? data.odds : [data.odds]; // oddsが配列でない場合は配列に変換
  ...
  return (
    <div className="bg-gray-400 grid grid-cols-1 divide-y text-black">
      {events.map((odds) => (
        ...
        <Link to={`/oddsDetail/${odds.id}`} key={`${odds.id}-${odds.marketName}`}>
